...
"""cs480_char_rnn_generation_tutorial.ipynb
...
class RNN_i(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(RNN_i, self).__init__()
...
class RNN_2(nn.Module):
    ...
    def forward(self, category, input, hidden, first):
        #Copy RNN_i for the first time step
        if not first:
...
def train(category_tensor, input_line_tensor, target_line_tensor):
    ...
    for i in range(input_line_tensor.size(0)):
        if i == 0:
          ...
          output, hidden = rnn(category_tensor, input_line_tensor[i], hidden, False)
...
rnn = RNN_3(n_letters, 128, n_letters)
...
import csv
with open("/content/drive/My Drive/CS680/A5/1b_3.csv", 'w') as csvFile:
